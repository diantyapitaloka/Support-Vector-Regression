...
- As mentioned at beginning of the module, apart from being able to solve classification problems, support vectors can also be used to predict continuous data, namely regression cases. Support Vector Regression (SVR) uses the same principles as SVM in the classification case.
- Geometric Strategy: While Linear Regression minimizes the sum of squared errors, SVR aims to fit the best line within a predefined threshold. It seeks a function $f(x)$ that has at most $\epsilon$ deviation from the actual targets for all training data.
...
- Decision Boundary Stability: Because the model is defined by the support vectors on the edge of the tube, it remains stable even if data points far from the boundary are moved. This characteristic provides SVR with a level of resilience against minor data perturbations compared to ordinary least squares.
